...
class IterativeRefinementGenerator(object):
    ...
    def generate(self, models, sample, prefix_tokens=None):
        ...
        # initialize
        encoder_out = model.forward_encoder(src_tokens, src_lengths,self.return_num,self.constraint_file)
    ...
    def generate_kpe(self, models, sample, prefix_tokens=None):
        ...
        if self.constrained_decoding:
            ...
            cached_examples = sample["cached_features"]
            encoder_out,constraints = model.forward_encoder(src_tokens, src_lengths,self.return_mode,self.return_num,self.constraint_file,cached_examples)
            
            #constraint_marks = [np.ones(len(const),dtype=np.int32) for const in constraints]
            constraint_marks = [torch.LongTensor(len(const)).fill_(1) for const in constraints]
    ...
    def generate_constraints(self, models, sample, prefix_tokens=None):
        ...
        cached_examples = sample["cached_features"]
        encoder_out,constraints = model.forward_encoder(src_tokens, src_lengths,cached_examples)
